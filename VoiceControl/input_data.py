...
class AudioProcessor(object):
  """Handles loading, partitioning, and preparing audio training data.
    ...
    Args:
      data_settings: data and model parameters, described at model_train_eval.py
  """

  def __init__(self, data_settings):
    wanted_words = data_settings.wanted_words.split(',')
    if data_settings.data_dir:
      self.data_dir = data_settings.data_dir
      if data_settings.split_data:
        self.maybe_download_and_extract_dataset(data_settings.data_url, self.data_dir)
        self.prepare_data_index(data_settings.silence_percentage,
                                data_settings.unknown_percentage, wanted_words,
                                data_settings.validation_percentage,
                                data_settings.testing_percentage, data_settings.split_data)
      else:
        self.prepare_split_data_index(wanted_words, data_settings.split_data)

      self.prepare_background_data()
    self.prepare_processing_graph(data_settings)
    ...
    with tf.get_default_graph().name_scope('data'):
      ...
      if data_settings.preprocess == 'raw':
        ...
        if data_settings.preprocess == 'average':
          ...
          window_size_ms = (data_settings.window_size_samples * 1000) / sample_rate
          window_step_ms = (data_settings.window_stride_samples * 1000) / sample_rate
      ...
      if data_settings.summaries_dir:
        self.summary_writer_ = tf.summary.FileWriter(
            data_settings.summaries_dir + '/data', tf.get_default_graph())
  ...
  def get_data(self, how_many, offset, data_settings, background_frequency,
    ...
    data = np.zeros((sample_count, data_settings.fingerprint_size))
    ...
    for i in xrange(offset, offset + sample_count):
      ...
      if use_background:
        ...
        if len(background_samples) <= data_settings.desired_samples:
          raise ValueError('Background sample is too short! Need more than %d'
                           ' samples but only %d were found' %
                           (data_settings.desired_samples, len(background_samples)))
        background_offset = np.random.randint(
            0,
            len(background_samples) - data_settings.desired_samples)
  ...
  def get_features_for_wav(self, wav_filename, data_settings, sess):
  ...
  def get_unprocessed_data(self, how_many, data_settings, mode):
